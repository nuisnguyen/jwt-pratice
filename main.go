...
// Token is for storing token information for correct login credential
type Token struct {
	Role        string `json:"role"`
	Email       string `json:"email"`
...
func ServerStart() {
	...
	err := http.ListenAndServe(":8080", handlers.CORS(handlers.AllowedHeaders([]string{"X-Reqeuested-With", "Access-Control-Allow-Origin", "Content-Type", "Authorization"}), handlers.AllowedMethods([]string{"GET", "POST", "PUT", "DELETE", "HEAD", "OPTIONS"}), handlers.AllowedOrigins([]string{"*"}))(router))
	if err != nil {
		log.Fatal(err)
	}
}
...
func SignUp(w http.ResponseWriter, r *http.Request) {
	...
	if dbuser.Email != "" {
		var err Error
		err = SetError(err, "Email is already in use")
...
func Signin(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		var err Error
		err = SetError(err, "Error is reading payload")
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(err)
		return
	}
